...
# Но потом вы заметили, что реализация методы .next_class() смотрится ну очень плохо
# И что бы написать хороший код, решили хранить инфо. о классе по другому
...
# В данном примере мы использовали приемы инкапсуляции, чтобы скрыть внутренюю реализацию класса


# Еще один пример инкапсуляции, с использование класса Вектор
import math
...
# Изменение атрибуты, вызывает метод: def x(self, x) с декоратором @x.setter
v1.x = -21
print('v1.x = ', v1.x)